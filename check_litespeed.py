...
"""
Author: Tony Ling
Reads and parses LiteSpeed live statistics file /tmp/lshttpd/.rtreport.
...
metrics['cache_hits_per_second']= ['Total cache hits per second', 'CACHE_HITS_PER_SEC:', 'greater']
metrics['total_cache_hits']= ['Total cache hits', 'TOTAL_CACHE_HITS:', None]
...
try:
	...
	with open("/tmp/lshttpd/.rtreport") as file:
		line = file.readline()
		if "Server/Open" in line:
			print("Error: Standard/Enterprise LiteSpeed version does not match, perhaps OpenLiteSpeed is installed instead")
			sys.exit(status_code)
		conn = ""
		max_conn = ""
		while line:
